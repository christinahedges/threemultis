...
def _run(clc, name):
    ...
    trace, mask = fit.fit_planets(clc, period_value=list(params['Period']),
                                                ...
                                                T_star=(params.loc[0, 'T_star'], params.loc[0, 'T_star_error']), ndraws=10000)

    samples = pm.trace_to_dataframe(trace, varnames=varnames)

    pickle.dump(samples, open('{}/results/{}_samples.p'.format(PACKAGEDIR, name), 'wb'))
    pickle.dump(mask, open('{}/results/{}_mask.p'.format(PACKAGEDIR, name), 'wb'))
    ...
    fig.savefig('{}/results/figures/{}_corner.png'.format(PACKAGEDIR, name), dpi=150, bbox_inches='tight')

    fig = utils.plot_folded_transits(clc, trace, mask, name);
    fig.savefig('{}/results/figures/{}.png'.format(PACKAGEDIR, name), dpi=150, bbox_inches='tight')
    ...
    if not os.path.isfile('{}/results/K2-198.fits'.format(PACKAGEDIR)):
        ...
        for idx, tpf in enumerate(tpfs):
            ...
            clc = fit.PLD(tpf, planet_mask=mask, trim=1, ndraws=1000, logrho_mu=np.log10(150),
                                      aperture=aper)
            pickle.dump(clc, open('{}/results/K2-198_{}.p'.format(PACKAGEDIR, idx), 'wb'))
            clcs.append(clc)

        clc = clcs[0].append(clcs[1])
        clc.to_fits('{}/results/K2-198.fits'.format(PACKAGEDIR), overwrite=True)
